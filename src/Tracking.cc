...
bool Tracking::TrackWithIMU(bool bMapUpdated)
{
	...
	if(mSensor!=System::STEREO)
		...
		th=7;
	// 将上一帧的MapPoints投影到currentFrame并看匹配个数, 3D-2D
...
cv::Mat Tracking::GrabImageMonoVI(const IMAGEData &image, const std::vector<IMUData> &vimu, const double &timestamp)
{
	mvIMUSinceLastKF.insert(mvIMUSinceLastKF.end(), vimu.begin(),vimu.end());
	
	mImGray = cv::Mat(image._row, image._col, CV_8U, cv::Scalar(150)); // bug
	
	if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET)
		mCurrentFrame = Frame(image,timestamp,vimu,mpIniORBextractor,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth);
	else
		mCurrentFrame = Frame(image,timestamp,vimu,mpORBextractorLeft,mpORBVocabulary,mK,mDistCoef,mbf,mThDepth,mpLastKeyFrame);
...
// 					cout << "相机跟踪 ..." << endl;
					// 没有恒速模型 / 刚重定位成功
					// mnLastRelocFrameId上一次重定位那一帧
					if(mVelocity.empty() || mCurrentFrame.mnId<mnLastRelocFrameId+2)
					{
						// 请上一帧的位姿作为当前帧的初始位姿
						// 通过BOW方式在参考帧中找当前帧特征点的匹配点
						// 优化每个特征点对应3D点重投影误差
						bOK = TrackReferenceKeyFrame();
					}
					else
...
// 				cout << "局部地图跟踪 ..." << endl;
		...
		*/
		...
		/*
		* 是否生成关键帧
		* 长时间没有插入关键帧 / 局部地图空闲 / 跟踪快丢失 |  地图跟踪的MapPoints比例少
		*/
		if(bOK)
		{
			// Update motion model
			...
			// 2.5 清除临时mappoints,这些mappoints在TrackWithMotionModel里生成
...
{
	// Tracking初始化为NULL
	// 以匹配100个特征点为界判断使用之前的初始化
	// 初始化目的:确定较为准确的3D点 / 通过3D-2D的方式统一尺度
	if(!mpInitializer)
	{
// 		cout << "初始化 initializer" << endl;		
		// Clear imu data
		mvIMUSinceLastKF.clear();

		// Set Reference Frame
		// 如果 Current Frame 特征点数目>100则新建初始化
		...
		if(mCurrentFrame.mvKeys.size()>100)
		{
			...
			mpInitializer =  new Initializer(mCurrentFrame,1.0,200); // 初始化
