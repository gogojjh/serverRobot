...
// 服务器端
Frame::Frame(const IMAGEData &image, const double &timeStamp, const std::vector<IMUData> &vimu, ORBextractor* extractor,ORBVocabulary* voc,
...
// 	ExtractORB(0,imGray);
	...
	mvKeys = image._vkeyPoints;
	N =  mvKeys.size();
	mDescriptors = image._descriptors.clone(); // bug
...
// 	cout << "mvKeys size " << mvKeys.size() << endl;
	...
	UndistortKeyPoints(); // 得到Frame::mvKeyUn
// 	cout << "mvKeyUn size " << mvKeysUn.size() << endl;
	...
	{
		ComputeImageBounds(image._row, image._col);

		mfGridElementWidthInv=static_cast<float>(FRAME_GRID_COLS)/static_cast<float>(mnMaxX-mnMinX);
		mfGridElementHeightInv=static_cast<float>(FRAME_GRID_ROWS)/static_cast<float>(mnMaxY-mnMinY);

// 		cout << mnMinX << " " << mnMaxX << " " << mnMinY << " " << mnMaxY << " " << mfGridElementWidthInv << " " << mfGridElementHeightInv << endl;
...
//     cout << "R: " << mDescriptors.rows << " C: " << mDescriptors.cols << endl;
...
//     cout << "Keypoints x " << x << " y " << y << endl;
    const int nMinCellX = max(0,(int)floor((x-mnMinX-r)*mfGridElementWidthInv));
//     cout << "nMinCellX " << nMinCellX <<  endl;
    if(nMinCellX>=FRAME_GRID_COLS)
        return vIndices;

    const int nMaxCellX = min((int)FRAME_GRID_COLS-1,(int)ceil((x-mnMinX+r)*mfGridElementWidthInv));
//     cout << "nMaxCellX " << nMinCellX <<  endl;
    if(nMaxCellX<0)
        return vIndices;

    const int nMinCellY = max(0,(int)floor((y-mnMinY-r)*mfGridElementHeightInv));
//     cout << "nMinCellY " << nMinCellX <<  endl;
    if(nMinCellY>=FRAME_GRID_ROWS)
        return vIndices;

    const int nMaxCellY = min((int)FRAME_GRID_ROWS-1,(int)ceil((y-mnMinY+r)*mfGridElementHeightInv));
//     cout << "nMaxCellY " << nMinCellX <<  endl;
    if(nMaxCellY<0)
        return vIndices;
...
void Frame::ComputeImageBounds(const int &row, const int &col)
{
	if(mDistCoef.at<float>(0)!=0.0)
	{
		cv::Mat mat(4,2,CV_32F);
		mat.at<float>(0,0)=0.0; mat.at<float>(0,1)=0.0;
		mat.at<float>(1,0)=col; mat.at<float>(1,1)=0.0;
		mat.at<float>(2,0)=0.0; mat.at<float>(2,1)=row;
		mat.at<float>(3,0)=col; mat.at<float>(3,1)=row;
