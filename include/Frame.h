...
public:
    ...
    // 服务器端
    Frame(const IMAGEData &image, const double &timeStamp, const std::vector<IMUData> &vimu, ORBextractor* extractor,ORBVocabulary* voc,
...
private: 
    ...
    // 计算图片校正后的边界    
    void ComputeImageBounds(const int &row, const int &col);
