...
void LocalMapping::Run()
{
	...
	while(1)
	{
		...
		//std::cout << "Mapping KeyFrame " << CheckNewKeyFrames()  << std::endl;
		// Check if there are keyframes in the queue
		if(CheckNewKeyFrames())
		{
// 			std::cout << "Mapping CheckNewKeyFrames" << std::endl;
			// Local Window also updated in below function
			// BoW conversion and insertion in Map
			// 更新MapPoints与KeyFrame的关联
			ProcessNewKeyFrame();
// 			std::cout << "Mapping ProcessNewKeyFrame" << std::endl;

			// Check recent MapPoints
			// 剔除mappoints
			MapPointCulling();
// 			std::cout << "Mapping MapPointCulling" << std::endl;
...
// 			std::cout << "Mapping CreateNewMapPoints" << std::endl;
			...
			if(!CheckNewKeyFrames())
			{
				...
				// mappoints 融合
				SearchInNeighbors();
// 				std::cout << "Mapping SearchInNeighbors" << std::endl;
			}
...
// 			std::cout << "Mapping InsertKeyFrame" << std::endl;
		}
		else if(Stop())
		{
// 			std::cout << "Mapping Stop" << std::endl;
		...
		ResetIfRequested();
// 		std::cout << "Mapping ResetIfRequested" << std::endl;

		// Tracking will see that Local Mapping is busy
		SetAcceptKeyFrames(true);
// 		std::cout << "Mapping SetAcceptKeyFrames" << std::endl;

		if(CheckFinish())
			break;
// 		std::cout << "Mapping CheckFinish" << std::endl;
	...
	SetFinish();
//  	std::cout << "Mapping SetFinish" << std::endl;
}
    ...
    for(vector<KeyFrame*>::iterator vit=vpLocalKeyFrames.begin(), vend=vpLocalKeyFrames.end(); vit!=vend; vit++)
    {
        ...
        for(size_t i=0, iend=vpMapPoints.size(); i<iend; i++)
        {
			...
			int nObs=0;
			    ...
			    // 尺度约束,要求mappoints在该共视KF的特征尺度大约(或近似于)其他关键帧的特征尺度
                            if(scaleLeveli<=scaleLevel+1)
                            {
                                nObs++;
				// 已经找到三个同尺度的关键帧可以观察这mappoints
                                if(nObs>=thObs)
                                    break;
                            }
                        }
