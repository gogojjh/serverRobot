...
public:
    ...
    // 服务器端: 构建Frame / 进入Track()进程
    cv::Mat GrabImageMonoVI(const IMAGEData &image, const std::vector<IMUData> &vimu, const double &timestamp);
...
protected:
    ...
    // 如果没有运动模型或TrackWithMotionModel失败就调用
    bool TrackReferenceKeyFrame();
    void UpdateLastFrame();
    
    // 将上一个frame里观测的3D map points投影到当前frame并进行匹配
    ...
    // 未初始化 / 唯一区别是提取特征是 mpORBextractorLeft 两倍
